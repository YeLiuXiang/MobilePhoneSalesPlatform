...
<c:set var="basePath" scope="request">${pageContext.request.contextPath}</c:set>
	...
	<input type="hidden" id="account"  value="${param.account}">
		<div id="container">
			<!--位置提醒-->
			<div class="posiRemind">您当前的位置：<span>我要推广</span> > <strong>广告展位竞拍</strong></div>
		<c:if test= "${empty requestScope.Object}"  >
			<center><h4>您尚未申请广告展位！</h4></center>
			</c:if>
			<c:if test= "${!empty requestScope.Object}"  >
			<div id="content">
					...
					<c:forEach var="anno" items="${requestScope.Object }"  varStatus="st">
							...
							<c:choose>
								<c:when test="${anno.status=='申请中'}">
									<td class="text-center"><button onclick="Delete(${anno.advId})"	class="btn btn-danger btn-sm">替换</button></td>
								</c:when>
								<c:otherwise>
	...
	<c:if test= "${empty requestScope.Object}"  >
		<input onclick="AddAdvertisingVerify()" type="button" class="btn btn-info btn-block" value="广告展位申请"/>
		</c:if>
		</div> 
