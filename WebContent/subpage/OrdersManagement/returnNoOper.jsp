...
<c:set var="basePath" scope="request">${pageContext.request.contextPath}</c:set>
	...
	<head>
		...
		<script src="${basePath}/js/returnOrder.js" type="text/javascript" charset="utf-8"></script>
		</head>
	...
	function send() {
		...
		window.location = "${basePath}/ReturnOrderServlet?method=ReturnOrderQueryById&searchType=0&id="+ id;
	}
	</script>
	<body>
		<div id="container">
			...
			<div class="row">
				...
				<!--下拉框-->
				<div class="btn-group">
				  	<button  onclick="window.location='${basePath}/ReturnOrderServlet?method=ReturnOrderQuery&searchType=0&currentpage=1&account=${sessionScope.account }'" type="button" class="btn btn-info">未处理</button>
				 	...
				 	<button onclick="window.location='${basePath}/ReturnOrderServlet?method=ReturnOrderQuery&searchType=2&currentpage=1&account=${sessionScope.account }'" type="button" class="btn btn-default">已拒绝</button>
				</div>
					...
					<c:forEach var ="retu" items="${requestScope.Object.list }">
						<tr>	<!--第1行-->
							...
							<td class="text-center">
								<button onclick="Agree(${retu.id})" class="btn btn-info btn-sm">同意</button>
								<button onclick="Refuse(${retu.id})"  class="btn btn-danger btn-sm">拒绝</button>
							</td>
						</tr>
						</c:forEach>
			...
			<footer>
					<ul class="pager">
	    				<li><a href="${basePath}/ReturnOrderServlet?method=ReturnOrderQuery&searchType=0&currentpage=${ requestScope.Object.previousPage}">上一页</a></li>
	    				<li><a href="${basePath}/ReturnOrderServlet?method=ReturnOrderQuery&searchType=0&currentpage=${ requestScope.Object.nextPage}">下一页</a></li>
					</ul>
					<form method = "post" action = "${basePath}/ReturnOrderServlet?method=ReturnOrderQuery&searchType=0"  class="navbar-form navbar-right" role="search" >
