...
<c:set var="basePath" scope="request">${pageContext.request.contextPath}</c:set>
	...
	<link rel="stylesheet" type="text/css" href="${basePath}/css/bootstrap.min.css"/>
		...
		<script src="${basePath}/js/orderSend.js" type="text/javascript" charset="utf-8"></script>
	</head>
	<body>
		<!--大容器begin-->
		<div id="container">
			...
			<div class="row">
				<!--订单类型-->
				<div class="btn-group">
				  	<button onclick="window.location='${basePath}/OrderServlet?method=OrderQuery&searchType=0&currentpage=1&account=${sessionScope.account }'" type="button" class="btn btn-default">未发货</button>
				  	<button onclick="window.location='${basePath}/OrderServlet?method=OrderQuery&searchType=1&currentpage=1&account=${sessionScope.account }'" type="button" class="btn btn-info">已发货</button>
					...
					<c:forEach  var = "order" items="${requestScope.Object.list }" varStatus="st">
						<c:choose>
							...
							<td class="text-center">
								...
								<button class="btn btn-danger btn-sm">物流</button>
							</td>
						</tr>
						</c:forEach>
			...
			<footer>
				<ul class="pager">
	    				<li><a href="${basePath}/OrderServlet?method=OrderQuery&searchType=1&account=${sessionScope.account }&currentpage=${ requestScope.Object.previousPage}">上一页</a></li>
	    				<li><a href="${basePath}/OrderServlet?method=OrderQuery&searchType=1&account=${sessionScope.account }&currentpage=${ requestScope.Object.nextPage}">下一页</a></li>
					</ul>
					<form method = "post" action = "${basePath}/OrderServlet?method=OrderQuery&searchType=1&account=${sessionScope.account }"  class="navbar-form navbar-right" role="search" >
