...
<c:set var="basePath" scope="request">${pageContext.request.contextPath}</c:set>
	...
	<body>
		<div id="container">
			<!--位置提醒-->
			<div class="posiRemind">您当前的位置：<span>销售统计</span> > <strong>销售量统计</strong></div>
			...
			<table class="table table-hover ">
				...
				<tbody>
				<c:set var="totalNumb" scope="session" value="${0 }"/>
				
					<c:forEach var="anno" items="${requestScope.Object}" varStatus="st">
					<c:set  var="totalNumb" scope="session" value="${sessionScope.totalNumb+anno.number }"/>
						<c:choose>
							...
							<td class="text-center">${anno.number }</td>
						</tr>
					</c:forEach>
				<tr class="success">
				<td class="text-center">销售总数量</td>
							<td class="text-center">${sessionScope.totalNumb }</td>
				</tr>
				</tbody>
			</table>
